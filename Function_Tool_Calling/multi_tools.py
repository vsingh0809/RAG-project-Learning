...
def run_tool(tool_call):
    ...
    print(f"  → LLM chose: {name}({args})")

    if name not in Tool_Registery:
        return f"Error: unknown tool {name}"
    
    result=Tool_Registery[name](**args)
    return str(result)
...
ask("what is the 1km in meter")
ask("What time is it in India right now?")
ask("How is the weather in Mumbai today?")
